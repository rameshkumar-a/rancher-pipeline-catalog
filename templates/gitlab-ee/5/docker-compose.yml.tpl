...
{{- if eq .Values.use_dns_name_check "true" }}
    labels:
     ...
     - "traefik.port=80"
     - "traefik.alias.fqdn=${gitlab_hostname},${domain_to_host_mattermost}"
     - "traefik.frontend.rule=Host:${gitlab_hostname},${domain_to_host_mattermost}"
     - "gitlab"
{{- end }}
    environment:
...
{{- end }}
{{- if (eq .Values.enable_gitlab_pages_check "true") }}
        pages_external_url "${protocol_to_host_gitlab_pages}${domain_to_host_gitlab_pages}"
        pages_nginx['enable'] = false
        gitlab_pages['external_http'] = ':8090' 
{{- end }}
{{- if eq .Values.enable_mattermost_check "true"}}
        mattermost_external_url '${protcol_to_host_mattermost}${domain_to_host_mattermost}'
{{- end}}
{{- if eq .Values.plantuml_enabled "true" }}
